...
util.parseError = function(errors) {
   ...
   if(errors.name == 'ValidationError') {
      ...
      parsed.emailId = { message:'This Email Id already exists!' };
   }
   else { parsed.unhandled = JSON.stringify(errors); }
   return parsed;
}
...
util.getArticleQueryString = function(req, res, next){
   res.locals.getArticleQueryString = function(isAppended=false, overwrites={}){    
      var queryString = '';
      var queryArray = [];
   ...
   for(var i=cloned.length-1; i>-1; i--){
      ...
      if(parentId){
         ...
         if(filtered.length){
            ...
            if(parent[childrenFieldName]) { parent[childrenFieldName].unshift(cloned[i]); }
            else { parent[childrenFieldName] = [cloned[i]]; }
         }
         cloned.splice(i,1);
      }
   }
